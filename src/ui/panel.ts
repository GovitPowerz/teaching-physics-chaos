...
import { lyapunovFit, separation, stateExtent, type LyapFit } from '../sim/ensemble'
...
// fit: pass the already-computed fit (revision-cached by createPanel) to
// skip recomputing it; omit to compute internally (existing callers unaffected)
const lambdaLine = (s: AppState, fit?: LyapFit | null): string => {
  const resolved = fit !== undefined ? fit
    : lyapunovFit(separation(s.ensemble.reference, s.ensemble.copies[0]),
      FIT_FRACTION * stateExtent(s.ensemble.reference))
  const lam = resolved ? fmt(resolved.lambda) : 'n/a'
  return `\u03b4(t) \u2248 \u03b40\u00b7e^(\u03bb\u00b7t), \u03bb \u2248 ${lam}`
}

export const formulasFor = (s: AppState, fit?: LyapFit | null): string[] => {
  switch (s.tab) {
    case 'lorenz':
      return [
...
interface FitCache { rev: number; fit: LyapFit | null }

export const createPanel = (store: Store) => {
  const el = document.createElement('div')
  let lastKey: string | null = null
  let fitCache: FitCache | null = null
  const render = () => {
    const s = store.get()
    // separation/stateExtent/lyapunovFit walk the whole recorded run (tens
    // of thousands of samples at the new horizons); render() fires every
    // rAF frame, so cache the fit per revision instead of recomputing it
    // while paused or merely scrubbing
    if (!fitCache || fitCache.rev !== s.revision) {
      const ref = s.ensemble.reference
      const series = separation(ref, s.ensemble.copies[0])
      const cutoff = FIT_FRACTION * stateExtent(ref)
      fitCache = { rev: s.revision, fit: lyapunovFit(series, cutoff) }
    }
    const lines = formulasFor(s, fitCache.fit)
